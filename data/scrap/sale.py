...
item_info = []
for i in [1,2,3]:    
    driver.get(BASE_URL + link_dict['sale'] + f"&page={i}")
    time.sleep(3) # 웹 페이지 로딩으로 인한 오류 및 서버 과부하 방지를 위해 3초간 프로세스를 일시 정지합니다.

    prdlist = driver.find_element(By.CLASS_NAME, 'prdList')
    box_list = prdlist.find_elements(By.CLASS_NAME, 'box')
...
with open('./data/item_sale.json', 'w', encoding='utf-8') as file:  
    json.dump(item_info, file, ensure_ascii=False, indent=2)
    
print(f'sale 카데고리 데이터 개수: {len(item_info)}')
